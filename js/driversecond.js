...
var app = Vue.createApp({
  ...
  computed: {
    disabledDates: function disabledDates() {
      var today = new Date();
      var disabledtoday = new Date(today);
      disabledtoday.setHours(today.getHours() + 3);
      return disabledtoday;
    },
    show_passenger_quantity: function show_passenger_quantity() {
      return "".concat(this.passenger_counter, " \u4F4D\u4E58\u5BA2\uFF0C").concat(this.pet_counter, " \u4F4D\u6BDB\u5B69");
    }
  },
  methods: {
    change: function change() {
      var msg_start = this.msg_start;
      var msg_end = this.msg_end;
      this.msg_start = msg_end;
      this.msg_end = msg_start;
    },
    car_click: function car_click(x) {
    ...
    PlusOne: function PlusOne(counter_name) {
      var car_class = 0;
      // this.passenger_counter = 1;
      // this.pet_counter = 1;
      if (this.car_menu === "轎車") {
        ...
        if (this.passenger_counter + this.pet_counter >= 3) {
          var dirver_Subtotal_icon_control = this.$refs.dirver_Subtotal_icon_control;
          dirver_Subtotal_icon_control.classList.add("limit");
          var pet_control = this.$refs.pet_Subtotal_icon_control;
    ...
    MinusOne: function MinusOne(counter_name, e) {
  ...
  mounted: function mounted() {
    ...
    var start_place = urlParams.get("start_place");
    var end_place = urlParams.get("end_place");
    var date_picker = urlParams.get("date-picker");
    var car_type = urlParams.get("car-type");
    var passenger_input = urlParams.get("passenger");
    var pet_input = urlParams.get("pet");
    if (start_place) {
...
var vm_2 = app.mount("#driver_app");

//顯示已加入購物車
var addCart = document.querySelector(".addCart");
var popup = document.querySelector(".ol-contentShop");
...
window.onload = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
  ...
  return _regeneratorRuntime().wrap(function _callee$(_context) {
    while (1) switch (_context.prev = _context.next) {
      case 0:
        _context.next = 2;
        return google.maps.importLibrary("places", "geometry");
      case 2:
        ...
        searchButton = document.getElementById("driverbtn");
...
function _calculateDistance() {
  _calculateDistance = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2() {
    var originInput, destinationInput, geocoder, originCoordinates, destinationCoordinates, request;
    return _regeneratorRuntime().wrap(function _callee2$(_context2) {
      while (1) switch (_context2.prev = _context2.next) {
        case 0:
          originInput = document.getElementById("start_place_input").value; // console.log(originInput);
          destinationInput = document.getElementById("end_place_input").value; // console.log(destinationInput);
          // 使用地點名稱來取得座標
          geocoder = new google.maps.Geocoder();
          _context2.next = 5;
          return getCoordinates(geocoder, originInput);
        case 5:
          originCoordinates = _context2.sent;
          _context2.next = 8;
          return getCoordinates(geocoder, destinationInput);
        case 8:
          destinationCoordinates = _context2.sent;
          if (originCoordinates && destinationCoordinates) {
            ...
            directionsService.route(request, function (response, status) {
              if (status === "OK") {
                ...
                var duration = response.routes[0].legs[0].duration.text; // 取得旅程所需時間
                // document.getElementById(
                //   "distanceDisplay"
                // ).textContent = `Distance: ${distance}`;
                document.getElementById("durationDisplay").textContent = "".concat(duration); // 顯示旅程所需時間
              } else {
...
function getCoordinates(_x, _x2) {
  return _getCoordinates.apply(this, arguments);
}
function _getCoordinates() {
  _getCoordinates = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee3(geocoder, location) {
